...
public final class RollupSeq implements iRowSeq {
  ...
  public void setRow(final KeyValue column) {
    ...
    if (need_count) {
      if (Bytes.memcmp(RollupQuery.SUM, column.qualifier(), 0, RollupQuery.SUM.length) == 0) {
        append(column, false);
      ...
      if (Bytes.memcmp(column.qualifier(), rollup_query.getRollupAggPrefix(), 0,
  ...
  public void addRow(final KeyValue column) {
    ...
    if (need_count) {
      ...
      if (Bytes.memcmp(RollupQuery.SUM, column.qualifier(), 0, 
          RollupQuery.SUM.length) == 0) {
        append(column, false);
      } else if (Bytes.memcmp(RollupQuery.COUNT, column.qualifier(), 0, 
          RollupQuery.COUNT.length) == 0) {
        append(column, true);
      } else {
        throw new IllegalDataException("Attempt to add a different aggrregate cell ="
            + column + ", expected aggregator either SUM or COUNT");
  ...
  private void append(KeyValue column, boolean is_count) {
    // for now assume we properly allocated our qualifiers
    if (is_count) {
      int offset = Internal.getOffsetFromQualifier(column.qualifier(), 
