...
final class StaticFileRpc implements HttpRpc {
  ...
  public StaticFileRpc() {
    URL queryuiURL =Thread.currentThread().getContextClassLoader().getResource("queryui");
    if (queryuiURL != null ) {
        staticroot = queryuiURL.getFile();
    } else {
        throw new RuntimeException("Cound not locate static root");
    }
  }
