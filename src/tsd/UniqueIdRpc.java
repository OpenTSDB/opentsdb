...
final class UniqueIdRpc implements HttpRpc {
	 ...
	 * strings with the {@code type} as their parameter and a comma separated list
	 * of values to assign UIDs to.
	 * <p>
	...
	private void handleAssign(final TSDB tsdb, final HttpQuery query) {
		...
		final Map<String, TreeMap<String, String>> response = new HashMap<String, TreeMap<String, String>>();
		...
		if (error_count < 1) {
			...
			query.sendReply(HttpResponseStatus.BAD_REQUEST, query.serializer()
					.formatUidAssignV1(response));
		}
	}
	...
	private void handleUIDMeta(final TSDB tsdb, final HttpQuery query) {
		...
		if (method == HttpMethod.GET) {
			...
			final UniqueIdType type = UniqueId.stringToUniqueIdType(query
					.getRequiredQueryStringParam("type"));
			try {
	...
	private void handleTSMeta(final TSDB tsdb, final HttpQuery query) {
		...
		if (method == HttpMethod.GET) {
			...
			if (tsuid == null) {
				throw new BadRequestException(
						"Metric was not specified. Provide tsuid or metric + tags.");
			}
			try {
	 ...
	 *           if a required value was missing or could not be parsed
	 */
	...
	/**
	 * Parses a query string "m=metric{tagk1=tagv1,...}" type query and returns a
	 * tsuid.
	 * 
	 * @param data_query
	 *          The query we're building
	 * @throws BadRequestException
	 *           if we are unable to parse the query or it is missing components
	 */
