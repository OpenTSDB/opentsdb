...
final class TsdbQuery implements Query {
  ...
  /**
   * Keep track of the latency we perceive when doing Scans on HBase. We want
   * buckets up to 16s, with 2 ms interval between each bucket up to 100 ms
   * after we which we switch to exponential buckets.
   */
  static final Histogram scanlatency = new Histogram(16000, (short) 2, 100);

  /**
   * Charset to use with our server-side row-filter. We use this one because it
   * preserves every possible byte unchanged.
   */
  ...
  /**
   * Values we may be grouping on. For certain elements in {@code group_bys}, we
   * may have a specific list of values IDs we're looking for. Those IDs are
   * stored in this map. The key is an element of {@code group_bys} (so a tag
   * name ID) and the values are tag value IDs (at least two).
   */
   ...
   * {@code group_bys} attribute. In the second case specifically, the {@code N}
   * values would be stored in {@code group_by_values}, the key in this map
   * being {@code name}.
   * 
   * @param tags
  ...
  /**
   * Finds all the {@link Span}s that match this query. This is what actually
   * scans the HBase table and loads the data into {@link Span}s.
   * 
   * @return A map from HBase row key to the {@link Span} for that row key.
   *         Since a {@link Span} actually contains multiple HBase rows, the row
   *         key stored in the map has its timestamp zero'ed out.
   * @throws HBaseException
   *           if there was a problem communicating with HBase to perform the
   *           search.
   * @throws IllegalArgumentException
   *           if bad data was retreived from HBase.
   */
   ...
   * 
   * @param spans
   *          The {@link Span}s found for this query ({@link #findSpans}). Can
   *          be {@code null}, in which case the array returned will be empty.
   * @return A possibly empty array of {@link SpanGroup}s built according to any
   *         'GROUP BY' formulated in this query.
   */
  private DataPoints[] groupByAndAggregate(final TreeMap<byte[], Span> spans) {
    ...
    if (group_bys == null) {
      ...
      final SpanGroup group = new SpanGroup(tsdb, getScanStartTime(),
          getScanEndTime(), spans.values(), rate, rateOptions,
          aggregator, sample_interval, downsampler);
    ...
    for (final Map.Entry<byte[], Span> entry : spans.entrySet()) {
      ...
      // LOG.info("Span belongs to group " + Arrays.toString(group) + ": " +
      // Arrays.toString(row));
      SpanGroup thegroup = groups.get(group);
      if (thegroup == null) {
        thegroup = new SpanGroup(tsdb, getScanStartTime(), getScanEndTime(),
            null, rate, rateOptions, aggregator, sample_interval,
            downsampler);
    ...
    // LOG.info("group for " + Arrays.toString(entry.getKey()) + ": " +
    // entry.getValue());
    // }
  ...
  Scanner getScanner() throws HBaseException {
    ...
    Bytes.setInt(end_row, (end_time == UNSET ? -1 // Will scan until the end
                                                  // (0xFFF...).
   ...
   * @param group_by
   *          A tag name ID.
   * @return {@code true} number if {@code tag} should be used next (because it
   *         contains a smaller ID), {@code false} otherwise.
   */
