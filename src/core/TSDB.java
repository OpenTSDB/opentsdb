...
public final class TSDB {
    ...
    /**
     * Normalization Plugin for fixing up tags
     */
    private NormalizePlugin normalize = null;
    ...
    public void initializePlugins(final boolean init_rpcs) {
        ...
        // load the normalize plugin if enabled
        if (config.getBoolean("tsd.normalize.enable")) {
            normalize = PluginLoader.loadSpecificPlugin(
                    config.getString("tsd.normalize.plugin"), NormalizePlugin.class);
            if (normalize == null) {
                throw new IllegalArgumentException("Unable to locate normalize plugin: " +
                        config.getString("tsd.normalize.plugin"));
            }
            try {
                normalize.initialize(this);
            } catch (Exception e) {
                throw new RuntimeException("Failed to initialize normalize plugin", e);
            }
            LOG.info("Successfully initialized normalize plugin [" +
                    normalize.getClass().getCanonicalName() + "] version: "
                    + normalize.version());
        } else {
            normalize = null;
        }
    ...
    /**
     * @return The optional normalize plugin
     */
    public NormalizePlugin getNormalizePlugin() {
