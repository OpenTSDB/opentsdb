    ...
    for (final KeyValue kv : row) {
      ...
      if (kv.qualifier()[0] == AppendDataPoints.APPEND_COLUMN_PREFIX) {
        ...
        while (idx < kv.value().length) {
          ...
          vLength = Internal.getValueLengthFromQualifier(kv.value(), idx);
          final byte[] q = new byte[qlength];
          final byte[] v = new byte[vLength];
          System.arraycopy(kv.value(),idx,q,0,qlength);
          System.arraycopy(kv.value(),idx + qlength,v, 0, vLength);
          idx += qlength + vLength;
