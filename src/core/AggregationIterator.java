 ...
 * a merged, aggregated view of multiple {@link Span}s. The data points in all
 * the Spans are returned in chronological order. Each time we return a data
 * point from a span, we aggregate it with the current value from all the other
 * Spans. If other Spans don't have a value at that specific timestamp, we do a
 * linear interpolation in order to estimate what the value of that Span should
 * be at that time.
 * <p>
 * All this merging, linear interpolation and aggregation happens in
 * {@code O(1)} space and {@code O(N)} time. All we need is to keep an iterator
 * on each Span, and {@code 4*k} {@code long}s in memory, where {@code k} is the
 * number of Spans in the group. When computing a rate, we need an extra
 * {@code 2*k} {@code long}s in memory (see below).
 * <p>
 * In order to do linear interpolation, we need to know two data points: the
 * current one and the next one. So for each Span in the group, we need 4 longs:
 * the current value, the current timestamp, the next value and the next
 * timestamp. We maintain two arrays for timestamps and values. Those arrays
 * have {@code 2 * iterators.length} elements. The first half contains the
 * current values and second half the next values. When a Span gets used, its
 * next data point becomes the current one (so its value and timestamp are moved
 * from the 2nd half of their respective array to the first half) and the
 * new-next data point is fetched from the underlying iterator of that Span.
 * <p>
 ...
 * </pre>
 * 
 * Since {@code current == 0}, the current data point has the value V1 and time
 * T1. Let's note that (V1, T1). Now this group has 2 Spans, which means we're
 * trying to aggregate 2 different series (same metric ID but different tags).
 * So The next value that this iterator returns needs to be a combination of V1
 * and V2 (assuming that T2 is less than T1). If our aggregation function is
 * "sum", we sort of want to sum up V1 and V2. But those two data points may not
 * necessarily be at the same time. T2 can be less than or equal to T1. If T2 is
 * greater than T1, we ignore V2 and return just V1, since we haven't reached
 * the time yet where V2 exist, so it's essentially as if it wasn't there. Say
 * T2 is less than T1. Summing up V1 and V2 doesn't make sense, since they
 * represent two measurements made at different times. So instead, we need to
 * find what the value V2 would have been, had it been measured at time T1
 * instead of T2. We do this using linear interpolation between the data point
 * (V2, T2) and the following one for that series, (V4, T4). The result is thus
 * the sum of V1 and the interpolated value between V2 and V4.
 * <p>
 * Now let's move onto the next data point. Assuming that T3 is less than T4, it
 * means we need to advance to the next point on the 1st series. To do this we
 * use the iterator it0 to get the next data point for that series and we end up
 ...
 * stored in a {@code long}. When a value is a floating point value, the bits of
 * the longs just need to be interpreted to get back the floating point value.
 * The way we keep track of the type is by using the most significant bit of the
 * timestamp (to avoid an extra array). This is fine since our timestamps only
 * really use 32 of the 64 bits of the long in which they're stored. When there
 * is no "current" value (1st half of the arrays depicted above), the timestamp
 * will be set to 0. When there is no "next" value (2nd half of the arrays), the
 * timestamp will be set to a special, really large value (too large to be a
 * valid timestamp).
 * <p>
 */
public class AggregationIterator implements SeekableView, DataPoint,
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(AggregationIterator.class);
	 ...
	 * @param rate
	 *            If {@code true}, the rate of the series will be used instead
	...
	public static AggregationIterator create(final List<Span> spans,
			...
			final Aggregator aggregator, final Interpolation method,
			final Aggregator downsampler, final long sample_interval_ms,
			final boolean rate, final RateOptions rate_options) {
		return create(spans, start_time, end_time, aggregator, method,
				downsampler, sample_interval_ms, rate, rate_options, null);
	}
	 ...
	 *            Specifies the optional additional rate calculation options.
	 * @param fill_policy
	 *            Policy specifying whether to interpolate or to fill missing
	...
	public static AggregationIterator create(final List<Span> spans,
		...
		for (int i = 0; i < size; i++) {
			...
			if (downsampler == null) {
				...
				it = spans.get(i).downsampler(start_time, end_time,
						sample_interval_ms, downsampler, fill_policy);
			}
			if (rate) {
	 ...
	 *            Interpolation method to use when aggregating time series
	 * @param downsampler
	 *            The downsampling specifier to use (cannot be null)
	 * @param query_start
	...
	public static AggregationIterator create(final List<Span> spans,
			...
			final long query_start, final long query_end, final boolean rate,
	 ...
	 * 
	 * @param iterators
	 *            An array of Seekable views of spans in a group. Ignored if
	 *            {@code null}. We modify the array while processing data
	 *            points.
	 * @param start_time
	 *            Any data point strictly before this timestamp will be ignored.
	 * @param end_time
		...
		for (int i = 0; i < size; i++) {
			...
			if (dp.timestamp() >= start_time) {
				...
				if (LOG.isDebugEnabled()) {
					LOG.debug(String.format("No DP in range for #%d: %d < %d",
							i, dp.timestamp(), start_time));
				}
				endReached(i);
				continue;
			}
	...
	public DataPoint next() {
		...
		// one that has the data point with the smallest timestamp since we want
		// to
		// return them in chronological order.
		current = -1;
		// If there's more than one Span with the same smallest timestamp, we'll
		// set this to true so we can fetch the next data point in all of them
		// at
		// the same time.
	...
	public double doubleValue() {
		if (!isInteger()) {
			...
			if (Double.isInfinite(value)) {
				throw new IllegalStateException("Got Infinity: " + value
						+ " in this " + this);
			}
			return value;
		}
	 ...
	 * 
	 * @param update_pos
	 *            Whether or not to also move the internal pointer {@link #pos}
	 *            to the index of the next value to aggregate.
	...
	public long nextLongValue() {
		if (hasNextValue(true)) {
			...
			if (rate) {
				throw new AssertionError("Should not be here, impossible! "
						+ this);
			}
			if (current == pos) {
				return y0;
	...
	public double nextDoubleValue() {
		if (hasNextValue(true)) {
			...
			if (rate) {
				// No LERP for the rate. Just uses the rate of any previous
				// timestamp.
				// If x0 is smaller than the current time stamp 'x', we just use
				// y0 as a current rate of the 'pos' span. If x0 is bigger than
				// the
				// current timestamp 'x', we don't go back further and just use
				// y0
				// instead. It happens only at the beginning of iteration.
				// TODO: Use the next rate the time range of which includes the
				// current
				// timestamp 'x'.
				return y0;
			}
			...
			final long x0 = timestamps[pos] & TIME_MASK;
			if (x == x0) {
				// LOG.debug("No lerp needed x == x0 (" + x + " == "+x0+") => "
				// + y0);
				return y0;
			}
			...
			if (x == x1) {
				// LOG.debug("No lerp needed x == x1 (" + x + " == "+x1+") => "
				// + y1);
				return y1;
			}
			if ((x1 & Const.MILLISECOND_MASK) != 0) {
	...
	public String toString() {
		return "SpanGroup.Iterator(timestamps=" + Arrays.toString(timestamps)
				+ ", values=" + Arrays.toString(values) + ", current="
				+ current + ", pos=" + pos + ", (SpanGroup: "
				+ toStringSharedAttributes() + "), iterators="
				+ Arrays.toString(iterators) + ')';
	}
