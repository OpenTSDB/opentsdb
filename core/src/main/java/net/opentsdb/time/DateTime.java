...
/**
 * Utility class that provides helpers for dealing with dates and timestamps. In particular, this
 * class handles parsing relative or human readable date/time strings provided in queries.
 *
 * @since 2.0
 */
public class DateTime {
  ...
  /**
   * Immutable cache mapping a timezone name to its object. We do this because the JDK's TimeZone
   * class was implemented by retards, and it's synchronized, going through a huge pile of code, and
   * allocating new objects all the time.  And to make things even better, if you ask for a TimeZone
   * that doesn't exist, it returns GMT!  It is thus impractical to tell if the timezone name was
   * valid or not.  JDK_brain_damage++; Note: caching everything wastes a few KB on RAM (34KB on my
   * system with 611 timezones -- each instance is 56 bytes with the Sun JDK).
   */
   ...
   * {@code 5m-ago}, {@code 1h-ago}, etc. See {@link #parseDuration}</li> <li>Absolute human
   * readable dates: <ul><li>"yyyy/MM/dd-HH:mm:ss"</li> <li>"yyyy/MM/dd HH:mm:ss"</li>
   * <li>"yyyy/MM/dd-HH:mm"</li> <li>"yyyy/MM/dd HH:mm"</li> <li>"yyyy/MM/dd"</li></ul></li>
   * <li>Unix Timestamp in seconds or milliseconds: <ul><li>1355961600</li> <li>1355961600000</li>
   * <li>1355961600.000</li></ul></li> </ul>
   *
   ...
   * attempt to validate the relative date.  So this function can return true on something that
   * looks like a relative date, but is actually invalid once we really try to parse it.
   ...
   * @param tzname Name of the timezone, or {@code null} in which case this function is a no-op.
