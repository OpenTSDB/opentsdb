...
public abstract class RTPublisher extends Plugin {
   ...
   * @param timestamp Timestamp as a Unix epoch in seconds or milliseconds (depending on the TSD's
   * configuration)
   * @param value Value for the data point
   * @param tags Tagk/v pairs
   * @param tsuid Time series UID for the value
   * @return A deferred without special meaning to wait on if necessary. The value may be null but a
