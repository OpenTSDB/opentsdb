...
//  
  ...
  /**
   * Whether or not this particular UID handler will return {@link NoSuchUniqueId} 
   * and {@link NoSuchUniqueName} exceptions when an ID or Name is not found
   * during lookup (true) or return nulls (false). 
   * @return True if the get methods return an exception, false if they return 
   * a null.
   * @since 3.0
   */
  public abstract boolean throwsNoSuchUniques();
  ...
  public Deferred<String> getName(final byte[] id) {
    return getName(id, null, null);
  }
  
  /**
   * Finds the name associated with a given ID.
   * @param id A non-null and non-empty byte array ID associated with that name.
   * @param trace An optional trace for use in tracing.
   ...
   * @since 3.0
   */
  public abstract Deferred<String> getName(final byte[] id, 
...
//  }
   ...
   * @since 3.0
   */
  public abstract Deferred<byte[]> getId(final String name, 
                                         final TsdbTrace trace, 
                                         final Span span);
...
//  
   ...
   * @param id A non-null time series ID used for validation and debugging.
   ...
   * @param name The name to lookup in the table or to assign an ID to.
   * @param id A non-null time series ID used for validation and debugging.
   * @param trace An optional trace for use in tracing the lookup and/or creation.
   * @param span An optional span for tracing.
   * @return a deferred resolving to a UID if found or created or an exception
   * from storage if something went wrong.
   * @since 3.0
   ...
   * @param ids A non-null list of time series IDs used for validation and 
   * debugging. Must be the same length as the names list.
...
//
   ...
   * @since 1.1
   */
  public abstract Deferred<List<String>> suggest(final String search, 
...
//
   ...
   */
  public abstract Deferred<Object> rename(final String oldname, 
...
//
   ...
   * @since 2.2
   */
  public abstract Deferred<Object> deleteAsync(final String name);
//    if (tsdb == null) {
