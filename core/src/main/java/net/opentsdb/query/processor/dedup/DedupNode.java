...
public class DedupNode extends AbstractQueryNode {
    ...
    public void onNext(QueryResult next) {
        ...
        for (TimeSeries ts : next.timeSeries()) {
            ...
            if (optional.isPresent()) {
                Iterator<TimeSeriesValue<? extends TimeSeriesDataType>> iterator = optional.get();
                TreeMap<TimeStamp, TimeSeriesValue<? extends TimeSeriesDataType>> tsMap = new TreeMap<>();
                while (iterator.hasNext()) {
                    TimeSeriesValue<? extends TimeSeriesDataType> tsValue = iterator.next();
                    tsMap.put(tsValue.timestamp(), tsValue);
                }
                dedupResult.results.add(new DedupTimeSeries(ts, tsMap.values().iterator()));
            }
        }
    ...
    private class DedupTimeSeries implements TimeSeries {
        ...
        public DedupTimeSeries(TimeSeries timeSeries, Iterator<TimeSeriesValue<? extends TimeSeriesDataType>> iterator) {
        ...
        @Override
        public Optional<Iterator<TimeSeriesValue<? extends TimeSeriesDataType>>> iterator(TypeToken<?> type) {
        ...
        public Collection<TypedIterator<TimeSeriesValue<? extends TimeSeriesDataType>>> iterators() {
            ...
            List<TypedIterator<TimeSeriesValue<? extends TimeSeriesDataType>>> iterators = timeSeries
                .iterators().stream().filter(ti -> ti.getType() == NumericType.TYPE)
                .collect(Collectors.toList());
            iterators
                .add(new TypedIterator(iterator, NumericType.TYPE));
            return iterators;
        }
