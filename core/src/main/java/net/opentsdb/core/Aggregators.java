...
public final class Aggregators {
  ...
  /**
   * Sums data points but will cause the SpanGroup to return a 0 if timesamps don't line up instead
   * of interpolating.
   */
  public static final Aggregator ZIMSUM = new Sum(
      Interpolation.ZIM, "zimsum");
  /**
   * Returns the minimum data point, causing SpanGroup to set <type>.MaxValue if timestamps don't
   * line up instead of interpolating.
   */
  public static final Aggregator MIMMIN = new Min(
      Interpolation.MAX, "mimmin");
  /**
   * Returns the maximum data point, causing SpanGroup to set <type>.MinValue if timestamps don't
   * line up instead of interpolating.
   */
  ...
  /**
   * Standard Deviation aggregator. Can compute without storing all of the data points in memory at
   * the same time.  This implementation is based upon a <a href="http://www.johndcook.com/standard_deviation.html">paper
   * by John D. Cook</a>, which itself is based upon a method that goes back to a 1962 paper by B.
   * P. Welford and is presented in Donald Knuth's Art of Computer Programming, Vol 2, page 232, 3rd
   * edition
   */
