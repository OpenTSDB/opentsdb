...
public final class TSSubQuery {
  ...
  /**
   * Runs through query parameters to make sure it's a valid request. This includes parsing the
   * aggregator, downsampling info, metrics, tags or timeseries and setting the local parsed fields
   * needed by the TSD for proper execution. If no exceptions are thrown, the query is considered
   * valid. <b>Note:</b> You do not need to call this directly as it will be executed by the {@link
   * TSQuery} object the sub query is assigned to.
   *
  ...
  /**
   * @return the raw downsampling function request from the user, e.g. "1h-avg"
   */
