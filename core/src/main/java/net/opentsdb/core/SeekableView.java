...
public interface SeekableView extends Iterator<DataPoint> {
  ...
  /**
   * Returns a <em>view</em> on the next data point. No new object gets created, the referenced
   * returned is always the same and must not be stored since its internal data structure will
   * change the next time {@code next()} is called.
   *
   * @throws java.util.NoSuchElementException if there were no more elements to iterate on (in which
   * case {@link #hasNext} would have returned {@code false}.
   */
  @Override
  DataPoint next();
   ...
   * @throws IllegalArgumentException if the timestamp is zero, or negative, or doesn't fit on 32
