...
public class Timestamp {
  ...
  public static long checkStartTime(long startTime) {
    checkArgument(startTime >= 0, "The timestamp must be positive and greater than zero but was %s", startTime);
   ...
   * @param endTime
   *
   * @throws IllegalArgumentException if either starTime or endTime is invalid or timespan is non positive. A timespan without an end is checked and allowed.
   */
  public static void checkTimeSpan(final long startTime, final long endTime) {
    ...
    checkArgument(endTime >= startTime || endTime == NOT_ENDED, "starttime was greater than endtime. Illegal negative timespan endtime - startim %s", endTime-startTime);
  }
}
