...
public class TestCassandraStore {
    ...
    private static final String METRIC_NAME_TREE = "cpu1";
    ...
    private static byte[]       TSUID_TREE;
    ...
    private void setUpData() throws Exception {
        ...
        name_uid.put(METRIC_NAME_TREE, store.allocateUID(
                METRIC_NAME_TREE, UniqueIdType.METRIC).joinUninterruptibly());
        ...
        TSUID_TREE = createTSUIDFromTreeUID(name_uid.get
                (METRIC_NAME_TREE),TAGK_UID_ONE, TAGV_UID_ONE);


        store.addPoint(TSUID_ONE, new byte[]{'d','1'},
                1356998400, (short)'a');
        store.addPoint(TSUID_ONE, new byte[]{'d','2'},
                1356998401, (short)'b');
        store.addPoint(TSUID_ONE, new byte[]{'d','2'},
                1357002078, (short)'b');

        store.addPoint(TSUID_TWO, new byte[]{'d','3'},
                1356998400, (short)'b');
    ...
    public void getName() throws Exception {
        ...
        name = store.getName(name_uid.get(METRIC_NAME_ONE), UniqueIdType.METRIC)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertTrue(name.isPresent());
        assertEquals(METRIC_NAME_ONE,name.get());

        name = store.getName(name_uid.get(METRIC_NAME_TWO), UniqueIdType.METRIC)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertTrue(name.isPresent());
        assertEquals(METRIC_NAME_TWO, name.get());

        name = store.getName(name_uid.get(METRIC_NAME_TREE), UniqueIdType.METRIC)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertTrue(name.isPresent());
        assertEquals(METRIC_NAME_TREE,name.get());

        name = store.getName(new byte[] {0, 0, 10}, UniqueIdType.METRIC)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertFalse(name.isPresent());


        name = store.getName(TAGK_UID_ONE, UniqueIdType.TAGK)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertTrue(name.isPresent());
        assertEquals(TAGK_NAME_ONE, name.get());

        name = store.getName(TAGV_UID_ONE, UniqueIdType.TAGV)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertTrue(name.isPresent());
        assertEquals(TAGV_NAME_ONE, name.get());
    ...
    public void getId() throws Exception {
        ...
        uid = store.getId(METRIC_NAME_ONE, UniqueIdType.METRIC)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertTrue(uid.isPresent());
        assertArrayEquals(name_uid.get(METRIC_NAME_ONE), uid.get());

        uid = store.getId(METRIC_NAME_TWO, UniqueIdType.METRIC)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertTrue(uid.isPresent());
        assertArrayEquals(name_uid.get(METRIC_NAME_TWO), uid.get());

        uid = store.getId(METRIC_NAME_TREE, UniqueIdType.METRIC)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertTrue(uid.isPresent());
        assertArrayEquals(name_uid.get(METRIC_NAME_TREE), uid.get());

        uid = store.getId("Missing", UniqueIdType.METRIC).joinUninterruptibly
                (CassandraConst.CASSANDRA_TIMEOUT);
        assertFalse(uid.isPresent());

        uid = store.getId(TAGK_NAME_ONE, UniqueIdType.TAGK)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertArrayEquals(TAGK_UID_ONE, uid.get());

        uid = store.getId(TAGV_NAME_ONE, UniqueIdType.TAGV)
                .joinUninterruptibly(CassandraConst.CASSANDRA_TIMEOUT);
        assertArrayEquals(TAGV_UID_ONE, uid.get());
