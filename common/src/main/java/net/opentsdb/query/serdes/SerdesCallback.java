...
/**
 * A callback passed to the serdes so that the caller knows when a serializer 
 * is finished with a particular partial time series and can close it out
 ...
 * @since 3.0
 */
public interface SerdesCallback {

  /**
   * Called by the serializer when it has finished dealing with the partial.
   * @param pts The non-null partial the sink is done with.
   */
  public void onComplete(final PartialTimeSeries pts);
  
  /**
   * Called by the serializer if an error happens when dealing with a partial.
