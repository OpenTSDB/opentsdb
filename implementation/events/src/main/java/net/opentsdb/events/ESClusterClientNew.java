...
import net.opentsdb.events.ESClient;
 ...
 * @since 3.0
 */
public class ESClusterClientNew extends BaseTSDBPlugin implements ESClient {
  private static final Logger LOG = LoggerFactory.getLogger(
    ESClusterClientNew.class);

  public static final String TYPE = "ESClusterClientNew";
  ...
  protected List<RestHighLevelClient> clients;
...
//          put("cluster.name", clusters[i]).build();

        RestHighLevelClient client = new RestHighLevelClient(
          RestClient.builder(new HttpHost((host_and_port[0]), host_and_port.length == 1 ? DEFAULT_PORT :
            Integer.parseInt(host_and_port[1]), "http"),
            new HttpHost((host_and_port[0]), host_and_port.length == 1 ? DEFAULT_PORT :
              Integer.parseInt(host_and_port[1]), "http")));

//        client.addTransportAddress(new TransportAddress(InetAddress.getByName
//          (host_and_port[0]), host_and_port.length == 1 ? DEFAULT_PORT :
//          Integer.parseInt(host_and_port[1])));
  ...
  public Deferred<Object> shutdown() {
    try {
      for (final RestHighLevelClient client : clients) {
        if (client != null) {
          client.close();
        }
      }
  ...
  public Deferred<Map<String, MultiSearchResponse>> runQuery(final Map<String,
    ...
    for (int i = 0; i < clients.size(); i++) {
      final RestHighLevelClient client = clients.get(i);
      final int idx = i;
      final Span local;
      if (child != null) {
      ...
      class FutureCB implements ActionListener<MultiSearchResponse> {
        ...
        public void onResponse(final MultiSearchResponse response) {
          ...
          tsdb.getStatsCollector().incrementCounter("es.client.query" +
              ".success", "endpoint", "events");
      ...
      try {
        MultiSearchRequest multi_search = new MultiSearchRequest();
        for (final Map.Entry<String, SearchSourceBuilder>
          ...
          searchRequest.source(search_source_builder.getValue());
          LOG.info("hello " + search_source_builder.getValue());
          LOG.info("hello 12 " + searchRequest);
//          final SearchRequestBuilder request_builder = client
//            .se
//            .setSearchType(SearchType.DEFAULT)
//            .setSource(search_source_builder.getValue())
//            .setTimeout(TimeValue.timeValueMillis(
//              tsdb.getConfig().getLong(QUERY_TIMEOUT_KEY)));
//
          multi_search.add(searchRequest);
...
//            }
//          }
          LOG.info("[" + clusters.get(idx) + "] Sending ES Query: "
            + query.toString());
        ...
        client.msearchAsync(multi_search, RequestOptions.DEFAULT, new FutureCB());
        //multi_search.execute(new FutureCB());
      } catch (Exception e) {
